...
# Modify by Sudipta Sarkar on date 16 February 2026 for MSN finetuning on video classification tasks.
...
def get_args_parser():
    ...
    parser.add_argument('--dataset', default='st2stv2', choices=list(DATASET_CONFIG.keys()), help='path to dataset file list')
    parser.add_argument('--duration', default=8, type=int, help='number of frames')
    parser.add_argument('--frames_per_group', default=1, type=int, help='[uniform sampling] number of frames per group; ' '[dense sampling]: sampling frequency')
    parser.add_argument('--threed_data', action='store_true', help='load data in the layout for 3D conv')
    parser.add_argument('--input_size', default=224, type=int, metavar='N', help='input image size')
    parser.add_argument('--disable_scaleup', action='store_true', help='do not scale up and then crop a small region, directly crop the input_size')
    ...
    parser.add_argument('--hpe_to_token', default=False, action='store_true', help='add hub position embedding to image tokens')
    # Model parameters
    ...
    parser.add_argument('--drop-path', type=float, default=0.0, metavar='PCT', help='Drop path rate (default: 0.1)')
    ...
    parser.add_argument('--rviews', type=float, nargs='+', default=[1, 0.975, 0.95, 0.925], help="List of float values for l2 (e.g., --l2 0.4 0.5)")
    parser.add_argument('--fviews', type=float, nargs='+', default=[0.9, 0.875, 0.85, 0.825], help="List of float values for l2 (e.g., --l2 0.4 0.5)")
    ...
    # Model specific
    parser.add_argument('--patch_drop', default=0.70, type=float, help='Percentage of patches to be dropped due to masking')
...
def main(args):
   ...
   #----------------------------------------------  Model creation ---------------------------------------------------------------- #
    ...
    print("Patch drop percentage:", float(args.patch_drop * 100), "%")
    ...
    if args.initial_checkpoint:
        print("Loading pretrained model from:",args.initial_checkpoint)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            memax_weight=args.memax_weight ,ent_weight=args.ent_weight, temporal_aug=args.temporal_aug)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Dino training and evaluation script', parents=[get_args_parser()])
