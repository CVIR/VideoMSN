...
from video_transforms import (GroupRandomHorizontalFlip, GroupOverSample,
                               ...
                               GroupNormalize, Stack, ToTorchFormatTensor, GroupRandomScale, GroupColorJitter, GroupGaussianBlur)
...
def get_augmentor(is_train: bool, image_size: int, mean: List[float] = None,
                  ...
                  modality: str = 'rgb', num_clips: int = 1, num_crops: int = 1, dataset: str = '', scales: List[float] = [1, .875, .75, .66], blur: bool = False):
    ...
    if modality == 'sound':
        ...
        augments = []
        augments += [GroupColorJitter(0.5,0.3,0)]
        if is_train:
            if blur:
...
def build_dataflow(dataset, is_train, batch_size, workers=36, is_distributed=False):
    ...
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle,
                                              num_workers=workers, pin_memory=True, sampler=sampler, drop_last=True, prefetch_factor=1)
