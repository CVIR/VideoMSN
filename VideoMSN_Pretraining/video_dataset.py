...
class VideoDataSet(data.Dataset):
    ...
    def __init__(self, root_path, list_file, num_groups=64, frames_per_group=1, sample_offset=0, num_clips=1,
                 ...
                 image_tmpl='{:05d}.jpg', rand_transform=None, focal_transform=None, is_train=True, test_mode=False, seperator=' ',
                 filter_video=0, num_classes=None, whole_video=False,
                 fps=29.97, audio_length=1.28, resampling_rate=24000, rand_views=1, focal_views=1,
                 duration=8, super_img_rows=1, rand_size=224, focal_size=96, temporal_aug=False):
        """
    ...
    def __getitem__(self, index):
        ...
        img_views += [self.rand_transform(images) for i in range(self.rand_views)]
        if self.temporal_aug:
            selected_indices = [0, 2, 4, 6, 7, 8, 10, 12, 14]
            selected_images = [images[i] for i in selected_indices]
            
            selected_indices_2x2 = [2, 6, 8, 12]
            selected_images_2x2 = [images[i] for i in selected_indices_2x2]
            img_views += [self.focal_transform(selected_images) for i in range(self.focal_views//2)]
            fc_transform = copy.deepcopy(self.focal_transform)
            fc_transform.transforms[1].input_size = [192, 192]
            img_views += [fc_transform(selected_images_2x2) for i in range(self.focal_views//2)]
        else:
            img_views += [self.focal_transform(images) for i in range(self.focal_views)]

        label = self.get_label(record)
        return img_views, label
...
class VideoDataSetOnline(VideoDataSet):
    ...
    def __init__(self, root_path, list_file, num_groups=8, frames_per_group=1, sample_offset=0,
        ...
        super().__init__(root_path, list_file, num_groups, frames_per_group, sample_offset,
                         ...
                         image_tmpl, rand_transform, focal_transform, is_train, test_mode, seperator,
                         filter_video, num_classes, whole_video, fps, audio_length, resampling_rate,
                         rand_views, focal_views,
                         duration, super_img_rows, rand_size, focal_size, temporal_aug)
